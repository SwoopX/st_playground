...
static int sqliteLoadAllSensorsCallback(void *user, int ncols, char **colval , char **colname)
{
    ...
    if (!sensor.id().isEmpty() && !sensor.name().isEmpty() && !sensor.type().isEmpty())
    {
        ...
        else if (sensor.type().endsWith(QLatin1String("Humidity")))
        {
            ...
            if (sensor.modelId() == QLatin1String("VOC_Sensor"))
            {
                item = sensor.addItem(DataTypeInt16, RStateEco2); // eCO2
                item->setValue(0);
                item = sensor.addItem(DataTypeUInt16, RStateVoc); // VOC value
                item->setValue(0);
            }
        ...
        else if (sensor.type().endsWith(QLatin1String("Water")))
        {
            ...
            else if (sensor.fingerPrint().hasInCluster(APPLIANCE_EVENTS_AND_ALERTS_ID))
            {
                clusterId = clusterId ? clusterId : APPLIANCE_EVENTS_AND_ALERTS_ID;
            }
